...
const Toolbar = (props: {
	...
	const { map, setTile, setPolygonCoords, select3D } = props
	...
	// If a selection is made on the 3D viewer need to remove any existing features
	// on the 2D view
	useEffect(() => {
	...
	// Handle click events on the map. If the pan tool is active we need to
	// highlight the grid polygon that was clicked, zoom the view to the extent
	// and set the grid row/col correctly so the corresponding 3D model can be
	// fetched.
	useEffect(() => {
		const handleClick = (e) => {
			// If dragPan is active then proceed
			if (activeTool === 'pan') {
				...
				if (gridFeature) {
					...
					// First reset style on any previously selected feature.
					...
					// TODO: Using 'as' to force the type check. It doesn't like the
					// possibility of a RenderFeature showing up.
					...
					// TODO: again had to force typings with 'as'
	...
	return (
		<div className="absolute left-0 top-0 z-20 flex flex-col m-1">
			...
			<button
				...
				} border shadow rounded-md p-2 mb-2 text-sm leading-5 font-medium bg-gray-200`}
				onClick={() => save(map)}
